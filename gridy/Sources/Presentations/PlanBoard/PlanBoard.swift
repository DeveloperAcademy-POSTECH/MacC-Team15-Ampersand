...
struct PlanBoard: Reducer {
    ...
    struct State: Equatable, Identifiable {
        ...
        var scheduleTemporarySelectedGridRange: SelectedGridRange?
        var drawingLine: DrawnLine?
        var shouldFinalizeDrawing = false
    ...
    enum Action: BindableAction, Equatable, Sendable {
        ...
        case setTemporarySelectedGridRange(SelectedGridRange?)
        case setDrawingLine(DrawnLine?)
        case setShouldFinalizeDrawing(Bool)
    ...
    var body: some Reducer<State, Action> {
        Reduce { state, action in
            ...
            case let .addCompletedLine(newLine):
                ...
                return .none
            case .setTemporarySelectedGridRange(let range):
                state.scheduleTemporarySelectedGridRange = range
                return .none
                
            case .setDrawingLine(let line):
                state.drawingLine = line
                return .none
            case .setShouldFinalizeDrawing(let shouldFinalize):
                state.shouldFinalizeDrawing = shouldFinalize
                return .none
